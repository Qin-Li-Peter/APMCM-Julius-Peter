...
图 2 进一步以三维形式展示 Pareto 前沿，其中三个坐标轴分别对应 $R^*$、$\Delta P^*$ 和 $\Theta^*$。由于三个指标均为越小越好，越靠近三维坐标低值区域的点代表综合性能越优；不同颜色表示不同针肋排数 $N_r$，用于观察离散结构变量对前沿分层的影响。

![三维 Pareto 前沿及关键折中解](figures/fig2_tradeoff_R_dP.png)

可以看到，Pareto 前沿整体呈现明显的空间折中关系：当 $R^*$ 降低时，$\Delta P^*$ 往往升高；当压降被压低时，热阻又会相应增大，同时 $\Theta^*$ 也随结构参数变化产生偏移。这说明芯片热管理结构优化不能只考虑散热能力，也必须同时考虑泵功代价和温度均匀性。图中的红色五角星为本文采用的膝点解，红色三角形为理想点法得到的辅助参考点。二者均位于前沿的折中区域，而不是任一单目标极值端点，说明膝点法得到的最终推荐方案并未过度偏向某一个指标。
